# 🎯 MICROTASK vs MACROTASK QUEUES - DEEP DIVE

Let me clear up the confusion once and for all!

---

## ⚠️ FIRST: THE TERMINOLOGY TRAP
...
## 📦 What Node Has Instead of "Macrotask Queue"
...
```js
setTimeout(() => {}, 0);  // → Goes to Timers queue
setInterval(() => {}, 100); // → Goes to Timers queue
```
...
```js
fs.readFile('file.txt', () => {});  // → Goes to Poll queue
fetch('url').then(() => {});        // → Resolves in Poll phase
http.get('url', () => {});          // → Goes to Poll queue
```
...
```js
socket.on('close', () => {});  // → Goes to Close queue
```

---

## 📦 What Node Has for "Microtask Queue"
...
await something();                  // → Continuation goes to microtask queue
queueMicrotask(() => {});          // → Goes to microtask queue
```
...
# 🎯 WHEN ARE QUEUES CREATED?

## ⚠️ IMPORTANT: Queues aren't "created" - they ALWAYS exist!
...
REGISTRATION TIME:
  ...
  Promise.then(() => {})  → Callback registered
...
QUEUE POPULATION:
  Timer expires     → Callback added to MACROTASK queue
  Promise resolves  → Callback added to MICROTASK queue
...
QUEUE POPULATION:
  ...
  File read done   → Added to POLL queue
  setImmediate     → Added to CHECK queue (immediately)
  nextTick         → Added to NEXTTICK queue (immediately)
  Promise resolved → Added to MICROTASK queue
...
DURING "1" EXECUTION:
  nextTick queue: [callback for "2"] ← Added NOW
  Microtask queue: [callback for "3"] ← Added NOW
  Timers queue: [callback for "4"] ← Added to NEXT loop

AFTER "1" COMPLETES:
  nextTick queue empties    → 2
  Microtask queue empties   → 3
...
|`setImmediate`|❌ N/A|Check|Immediately|
|`process.nextTick`|❌ N/A|nextTick|Immediately|
...
|`fetch` callback|Macrotask|Poll (resolve) → Microtask (callback)|When network completes|
...
- Browser: ALL micros → ONE macro → ALL micros → ONE macro
- Node: ALL micros → ALL in phase → ALL micros → next phase
